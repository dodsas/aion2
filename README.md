...
루드라·침식·무스펠 파티를 모집한다. 기간은 **수요일~화요일 한 주**(오늘 기준 *이번주/다음주*)
단위로 잡고, 총 판수를 정한다. **내 캐릭터 카드를 드래그**해 신청하며, 신청·참석은
**그룹(인원) 단위**로 관리한다.
- 정원: **인원(그룹) ≤ 10**, **캐릭 합계 ≤ 판수 × 10** (예: 2판 → 최대 20캐릭/10인원).
- 카드에 **프로필 + 전투력**(677,000 → `677`, 천 단위 절삭) 노출.
- 달력은 **수·목·금·토·일·월·화** 순, 각 칸에 실제 날짜 명시. 그룹 선택 후 참석 가능일을
  클릭해 찍고, 날짜별 신청 현황(누가 참석하는지)을 한눈에 관리.
...
브라우저에 남아 있던 `localStorage` 데이터는 **첫 접속 시 서버로 자동 이관**된다.

> ⚠️ 현재 인증이 없어 배포 인스턴스 전체가 **하나의 공유 데이터셋**을 본다(소규모 길드용).
...
`render.yaml` 이 Docker 웹 서비스(무료)를 정의한다.

1. 리포를 GitHub 에 푸시.
2. Render → **New + → Blueprint** → 이 리포 선택 → **Apply** (`render.yaml` 자동 인식).
3. 서비스 → **Environment** 에서 `TURSO_DATABASE_URL`, `TURSO_AUTH_TOKEN` 입력 → 자동 재배포.

> Render 무료 플랜은 **디스크가 비영속**이라 재배포/슬립 시 로컬 SQLite 폴백 데이터가 사라진다.
> 데이터 영속이 필요하면 **Turso 설정이 사실상 필수**다. 무료 웹 서비스는 15분 무접속 시 슬립하고
> 다음 요청에서 콜드 스타트한다. `PORT` 는 Render 가 자동 주입한다(`server.py` 가 읽음).
